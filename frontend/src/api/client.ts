...
// Case-scoped route families: What-If config/models/dashboard, and the Soft
// Sensor endpoints whose model registry / saved_models are also per-case
// (see the case-isolation plan's "Full-stack case isolation scope"). Every
// request to one of these gets ?case_id=<active case> attached automatically
...
const CASE_SCOPED_URL_PREFIX = /^\/(what-if|overview|training|predict)(\/|$)/
